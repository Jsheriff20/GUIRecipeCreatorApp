...
void GettingIngredients::GetIngredient(vector<double>*finalIngredientsAmounts, vector<string>*finalIngredientsMeasurement,
	vector<string>*ingredients, vector<string>*descriptionForIngredients, int *ingredientNum, bool editingExistingIngredientsListOrNot,
	int editOrDeleteItemLocation) {
	...
	while (true) {
		...
		*ingredientNum += 1;
		...
		if (editingExistingIngredientsListOrNot == false) {
			...
			cout << endl << "Item: " << amount << " " << measurementType << " " << ingredient << " " << description <<
				"    has been added to list." << endl << endl << endl << endl;
		}
		else {
